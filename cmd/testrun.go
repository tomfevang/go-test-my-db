...
// even on error.
func runTestPipeline(db *sql.DB, schema string, cfg *config.Config, schemaFile string, rows, batchSize, workers, minChildren, maxChildren, maxRowsCap int, seedTables []string) ([]TestResult, int, error) {
	// Parse DDL file.
